...
//使用 Bouncy Castle 提供的密码学算法和协议
    ...
    private static SecretKeySpec generateKey(final String password)
        ...
        final MessageDigest digest = MessageDigest.getInstance(HASH_ALGORITHM);//使用md5获取一个消息摘要对象
        byte[] bytes = password.getBytes("UTF-8");
        digest.update(bytes, 0, bytes.length);//将密码按照 UTF-8 编码为字节数组 bytes，然后将该数组添加到 MD5 消息摘要对象中
        byte[] key = digest.digest();//处理数据后得到摘要字节数组，作为 AES 加密算法的密钥

        return new SecretKeySpec(key, "AES");//创建出一个适用于 AES 算法的秘钥，SecretKeySpec 是专门用于包装对称秘钥（AES）的类
    }
    ...
    public static String encrypt128Key(final byte[] password, String message)
        ...
        try {
            ...
            return Base64.encodeBase64String(cipherText);//cipher得到的是二进制形式的密文，为了方便传递和存储，通常需要对其进行 Base64 编码转换成字符串的形式。
    ...
    public static String encrypt(final String password, String message)
        ...
        try {
            final SecretKeySpec key = generateKey(password);//获取密码密钥
            byte[] cipherText = encrypt(key, ivBytes, message.getBytes(CHARSET));
            // NO_WRAP is important as was getting \n at the end
    ...
    public static byte[] encrypt(final SecretKeySpec key, final byte[] iv, final byte[] message)
            throws GeneralSecurityException {
        final Cipher cipher = Cipher.getInstance(AES_MODE);//创建一个cipher加密对象，加密模式AES
        IvParameterSpec ivSpec = new IvParameterSpec(iv);
        cipher.init(Cipher.ENCRYPT_MODE, key, ivSpec);

        return cipher.doFinal(message);//对明文数据进行 AES 加密，并返回加密后的密文。
    }
